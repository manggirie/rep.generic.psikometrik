...
var list = SesiUjians.Where (su => su.Status == "Belum Ambil" && su.NamaUjian == "HLP");
var questions = Soalans.Where (s => s.NamaUjian == "Ujian Khas Hadiah Latihan Persekutuan(HLP)").Select (s => s.Json.DeserializeFromJson<Bespoke.epsikologi_soalan.Domain.Soalan>()).ToList();
