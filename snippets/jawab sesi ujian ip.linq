...
var list = SesiUjians.Where (su => su.Status == "Belum Ambil" && su.NamaUjian == "IP");
var questions = Soalans.Where (s => s.NamaUjian == "Indeks Personaliti (IP)").Select (s => s.Json.DeserializeFromJson<Bespoke.epsikologi_soalan.Domain.Soalan>()).ToList();
