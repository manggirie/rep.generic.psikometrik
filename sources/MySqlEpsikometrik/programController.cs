...
{
   [RoutePrefix("api/epsikometrik/program")]
   public partial class programController : ApiController
   {
       ...
       public async Task<HttpResponseMessage> Remove(int id)
       {
            ...
            var context = new programAdapter();
       ...
       [Route("")]
       [HttpPost]
       public async Task<HttpResponseMessage> Insert([FromBody]program item)
       {
       ...
       public async Task<object> List(string filter = null, int page = 1, int size = 40, bool includeTotal = false, string orderby = null)
       {
           ...
           if (size > 200)
            ...
            var translator = new OdataSqlTranslator<program>(null,"program" ){Schema = "epsikometrik"};
            ...
            {
                var translator2 = new OdataSqlTranslator<program>(null, "program"){Schema = "epsikometrik"};
                ...
                if (count >= lo.ItemCollection.Count())
                    nextPageToken = string.Format(
                        "/api/epsikometrik/program/?filter={0}&includeTotal=true&page={1}&size={2}", filter, page + 1, size);
            }

            string previousPageToken = string.Format("/api/epsikometrik/program/?filter={0}&includeTotal=true&page={1}&size={2}", filter, page - 1, size);
            if(page == 1)
       ...
       [Route]
       [HttpPut]
       public async Task<HttpResponseMessage> Save([FromBody]program item)
       {
